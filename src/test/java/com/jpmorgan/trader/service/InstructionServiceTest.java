...
public class InstructionServiceTest extends TestCase {
	public void testReturnSameDateIfItIsAlreadyWorkingDate() throws ParseException {
		...
		Date workingDate = instructionService.getActualSettlementDate(date, "SAT,SUN");
		assertTrue(workingDate.compareTo(new SimpleDateFormat("yyyy/MM/dd", Locale.ENGLISH).parse("2018/09/21")) == 0);
	}
	...
	public void testReturnNextWorkingDateIfItIsWeekend() throws ParseException {
		...
		Date workingDate = instructionService.getActualSettlementDate(date, "SAT,SUN");
		assertTrue(workingDate.compareTo(new SimpleDateFormat("yyyy/MM/dd", Locale.ENGLISH).parse("2018/09/24")) == 0);
	}
}

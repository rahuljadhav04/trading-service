...
/**
 * This class is acting as 4 components.
 * Calling JMS listener component as we won't have JMS here.
 * It would get Mock instructions from Mockdata and send to instruction listener
 * where instruction listener would save instructions to database
 * It would then call Report saver component to save data into report format.(to avoid JOIN queries to fetch report)
 * It real world it can be batch job which would daily save data into report format.
 * 
 * Then it would call report generator, which would simply get data from report format database and siaply it in excel format.
 * Here it is just printing to console.
 * 
 * It would also call cache initializer to initialize cache like currency to weekend mapping
 * 
